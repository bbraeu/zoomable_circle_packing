...
export default class BubbleChart extends Component {
	...
	prepareChart() {
		...
		svg.style('background', this.color(-1)).on('click', () => {
			this.zoom(this.root);
		});

		this.root = d3.hierarchy(this.data).sum((value) => value.size).sort((a, b) => {
			return b.value - a.value;
		});
	...
	handleClick = () => {
		var newNode = d3.hierarchy({ name: 'test' });

		this.root.children.push(newNode);
		this.root.data.children.push(newNode.data);

		this.drawNodes();
		console.log('New node pushed ' + this.root.children.length);
	};
